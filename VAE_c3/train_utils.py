...
def loss_function(recon_x, x, mask, z, z_prior):
    ...
    mse_f = torch.sum(masked_mse) / (torch.sum(mask))
...
def train(model, device, train_loader, optimizer, epoch, log_interval, mask):
    ...
    for batch_idx, data in enumerate(train_loader):
        data = data.to(device)
        optimizer.zero_grad()
        recon_batch, z, z_prior = model(data)
        loss = loss_function(recon_batch, data, mask, z, z_prior)
        loss.backward()
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            for i in range(data.size(0)):
                loss = loss_function(recon_batch[i:i+1], data[i:i+1], mask, z[i].unsqueeze(0) , z_prior[i].unsqueeze(0)).item()
                test_details.append({
